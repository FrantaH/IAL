...
**/
    ...
    PtrReplaced->Key = (*RootPtr)->RPtr->Key;                  //naplnění mazaného uhlu tím nejpravějším zleva
    PtrReplaced->BSTNodeCont = (*RootPtr)->RPtr->BSTNodeCont;
    tBSTNodePtr tmp = (*RootPtr)->RPtr->LPtr;                     //dočasné uložení LPtr toho nejpravějšího (přesouvaného)
    free((*RootPtr)->RPtr);                                    //uvolnění místa toho nejpravějšího
    (*RootPtr)->RPtr=tmp;                                      //přenesení levého podstromu toho nejpravějšího na jeho "otce"
    }
